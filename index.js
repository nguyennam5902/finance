...
app.get('/setting', async (_req, res) => {
    ...
    if (lib.isValidString(app.get(`username`))) {
        ...
        res.render('setting', {
            isLogin: true, main: `<div class="form-group"><label for="input-field" style="display: inline-block;">Username: ${app.get(`username`)}\t\t</label>
...
app.get('/recharge', (_req, res) => {
    if (lib.isValidString(app.get(`username`))) {
        res.render('recharge', { isLogin: true });
    }
    else { res.redirect('/login') }
});
